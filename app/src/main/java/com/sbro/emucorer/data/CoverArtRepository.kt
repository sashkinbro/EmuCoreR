...
class CoverArtRepository(context: Context) {
    ...
    companion object {
        ...
        const val DEFAULT_COVER_BASE_URL = "https://raw.githubusercontent.com/xlenore/psx-covers/main/covers/default"
        const val DEFAULT_COVER_3D_BASE_URL = "https://raw.githubusercontent.com/xlenore/psx-covers/main/covers/3d"
    ...
    fun downloadCover(
        ...
        val flightKey = "$style|$coverBaseUrl|$normalizedSerial"
        ...
        return try {
            downloadCoverResolved(serial, normalizedSerial, style, coverBaseUrl).also {
                flight.result = it
            }
        } finally {
            synchronized(downloadFlightsLock) {
    ...
    private fun downloadFromUrl(
        ...
        return try {
            ...
            connection.setRequestProperty("User-Agent", "Mozilla/5.0 (EmuCoreR)")
            connection.connect()
