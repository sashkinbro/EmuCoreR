...
private val JAPANESE_IMPORT_SERIAL_PREFIXES = listOf("SCPS", "SLPS", "SLPM", "SIPS", "ESPM")
...
enum class EmuAchievementMetric {
    ...
    MatchingGamesCount,
    MetalGearSolidMinutes,
    FinalFantasySevenMinutes,
    CrashBandicootMinutes
}
...
object EmuCoreRAchievementCatalog {
    const val VERSION = 2
    ...
    val definitions = buildList {
        ...
        EmuAchievementDefinition("import_debut", R.string.achievement_import_debut_title, R.string.achievement_import_debut_description, EmuAchievementMetric.ImportGames, 1, 25),
        ...
        EmuAchievementDefinition("metal_gear_solid_secret", R.string.achievement_metal_gear_solid_title, R.string.achievement_metal_gear_solid_description, EmuAchievementMetric.MetalGearSolidMinutes, 120, 50, hidden = true),
        EmuAchievementDefinition("final_fantasy_seven_secret", R.string.achievement_final_fantasy_seven_title, R.string.achievement_final_fantasy_seven_description, EmuAchievementMetric.FinalFantasySevenMinutes, 120, 50, hidden = true),
        EmuAchievementDefinition("crash_bandicoot_secret", R.string.achievement_crash_bandicoot_title, R.string.achievement_crash_bandicoot_description, EmuAchievementMetric.CrashBandicootMinutes, 90, 50, hidden = true)
        ))
        ...
        milestones("import_library", EmuAchievementMetric.ImportGames, listOf(2, 3, 5, 10, 20), R.string.achievement_import_library_title, R.string.achievement_import_library_description)
        milestones("import_minutes", EmuAchievementMetric.ImportMinutes, listOf(30, 60, 180, 300, 600, 1_500), R.string.achievement_import_time_title, R.string.achievement_import_time_description)
        milestones("import_sessions", EmuAchievementMetric.ImportSessions, listOf(1, 5, 10, 25, 50), R.string.achievement_import_sessions_title, R.string.achievement_import_sessions_description)

        val topGames = listOf(
            "Metal Gear Solid" to "metal gear solid", "Final Fantasy VII" to "final fantasy vii",
            "Crash Bandicoot" to "crash bandicoot", "Tekken 3" to "tekken 3",
            "Gran Turismo" to "gran turismo", "Resident Evil" to "resident evil",
            "Silent Hill" to "silent hill", "Spyro the Dragon" to "spyro",
            "Tomb Raider" to "tomb raider", "Castlevania: Symphony of the Night" to "symphony of the night",
            "Tony Hawk's Pro Skater" to "tony hawk", "Crash Team Racing" to "crash team racing",
            "Final Fantasy IX" to "final fantasy ix", "Legacy of Kain: Soul Reaver" to "soul reaver"
        )
        ...
        listOf(
            ...
            "Crash Bandicoot" to "crash bandicoot", "Spyro" to "spyro",
            "Resident Evil" to "resident evil", "Tekken" to "tekken",
            "Gran Turismo" to "gran turismo", "Tomb Raider" to "tomb raider"
        ).forEach { (series, term) ->
...
class EmuAchievementRepository(context: Context) {
    ...
    private suspend fun evaluateAndSync(snapshot: AchievementSnapshot): List<EmuAchievementState> {
        ...
        val candidates = EmuCoreRAchievementCatalog.definitions.filter { definition ->
        ...
        val newlyUnlocked = if (candidates.isEmpty()) emptyList() else runCatching { firestore.runTransaction { transaction ->
            ...
            unreadUnlocks.filterNot(AchievementSyncCandidate::unlockExists).map { candidate ->
                ...
                transaction.set(candidate.unlockReference, mapOf(
                    ...
                    "catalogVersion" to EmuCoreRAchievementCatalog.VERSION,
        ...
        val states = EmuCoreRAchievementCatalog.definitions.map { definition ->
        ...
        runCatching {
            firestore.collection(PUBLIC_PROFILES).document(uid).set(
                mapOf(
                    ...
                    "achievementCatalogVersion" to EmuCoreRAchievementCatalog.VERSION,
    ...
    private suspend fun loadPublic(uid: String, profile: PlayerProfile): List<EmuAchievementState> {
        ...
        return EmuCoreRAchievementCatalog.definitions.map { definition ->
            EmuAchievementState(
                definition = definition,
    ...
    private data class AchievementSnapshot(
        ...
        fun progressFor(definition: EmuAchievementDefinition): Long = when (definition.metric) {
            ...
            EmuAchievementMetric.ImportGames -> importGames().size.toLong()
            EmuAchievementMetric.ImportMinutes -> importGames().sumOf { it.totalPlayTimeMs } / 60_000L
            EmuAchievementMetric.ImportSessions -> importGames().sumOf { it.sessions }
            ...
            EmuAchievementMetric.MetalGearSolidMinutes -> minutesForTitle("metal gear solid")
            EmuAchievementMetric.FinalFantasySevenMinutes -> minutesForTitle("final fantasy vii")
            EmuAchievementMetric.CrashBandicootMinutes -> minutesForTitle("crash bandicoot")
        }

        private fun importGames() = games.filter { game ->
            val serial = game.serial.orEmpty().uppercase(Locale.ROOT)
            JAPANESE_IMPORT_SERIAL_PREFIXES.any(serial::startsWith)
        }
    ...
    private companion object {
        ...
        const val FEED = "feed"
        const val CHANNEL_ID = "emucorer_achievements"
