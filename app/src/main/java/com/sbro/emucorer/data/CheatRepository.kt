...
class CheatRepository(private val context: Context) {
    ...
    /** Core-format (PCSX `.cht`) file matching the active `.pnach` target, if written. */
    ...
    private fun coreCheatFile(pnachFile: File): File =
        File(pnachFile.parentFile, "${pnachFile.nameWithoutExtension}.cht")
    ...
    private fun convertCheatBlock(block: CheatBlock): List<String>? {
        ...
        block.lines.forEach { rawLine ->
            ...
            when (match.groupValues[2].lowercase(Locale.US)) {
                "0", "byte" -> codeLines += String.format(
                    Locale.US, "%08X %02X", 0x30000000L or ramAddress, value and 0xFFL
                )
                "1", "short" -> codeLines += String.format(
                    Locale.US, "%08X %04X", 0x80000000L or ramAddress, value and 0xFFFFL
                )
                "2", "word" -> {
                    val base = 0x80000000L or ramAddress
                    codeLines += String.format(
                        Locale.US, "%08X %04X", base + 2L, (value shr 16) and 0xFFFFL
                    )
                    codeLines += String.format(
                        Locale.US, "%08X %04X", base, value and 0xFFFFL
                    )
                }
                else -> return null
            }
