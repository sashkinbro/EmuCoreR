package com.sbro.emucorer.data.drive
...
import com.sbro.emucorer.data.drive.DriveBackupArchive.Companion.hex
...
class DriveBackupApi(private val context: Context, private val email: String) {
    ...
    suspend fun list(): List<DriveBackupEntry> {
        ...
        var next = ""
        do {
            val query = "trashed = false and appProperties has { key='emucorerBackup' and value='1' } and appProperties has { key='complete' and value='1' }"
    ...
    private suspend fun folder(): String {
        val q = "trashed = false and mimeType = 'application/vnd.google-apps.folder' and appProperties has { key='emucorerFolder' and value='1' }"
        ...
        return json("files?fields=id", "POST", JSONObject().put("name", "EmuCoreR Backups")
            .put("mimeType", "application/vnd.google-apps.folder")
            .put("appProperties", JSONObject().put("emucorerFolder", "1"))).getString("id")
    }
    ...
    suspend fun upload(snapshot: DriveSnapshot, verifying: suspend () -> Unit = {}, progress: suspend (Long, Long) -> Unit): String {
        ...
        val props = JSONObject().put("emucorerBackup", "1").put("complete", "0")
        ...
        if (url.isBlank()) {
            val metadata = JSONObject().put("id", id).put("name", "EmuCoreR-${snapshot.manifest.getLong("createdAt")}.zip.uploading")
        ...
        json("files/${enc(id)}", "PATCH", JSONObject().put("appProperties", props.put("complete", "1"))
            .put("name", "EmuCoreR-${snapshot.manifest.getLong("createdAt")}.zip"))
        sessionFile.delete()
        return id
    }
